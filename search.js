...
module.exports = function (query, latlonProvider) {
	...
	if (found)
		return es.readArray(found)
			.pipe(es.mapSync(function (index) { // TODO check if Sync is removable (create a bug for now)
				var item = data[index];
				return [item, getScore(query, item, latlonProvider())]; // scoring is poorly implemented
			}))
			.pipe(sort(cmp))
...
var formatPropertyDebug = function (item) {
	var result = formatProperty(item);
	result.ascii = item[0].ascii;
	result.alt = item[0].altNames;
	return result;
...
var getScore = function (str, item, latlon) {
	var score = str.length / item.ascii.length;
	if (latlon)
...
//	return Math.sqrt(x * x + y * y); // TODO remove sqrt and update distance categories accordingly
	return x * x + y * y;
}
