	...
	describe('Collections', function () {
		...
		it('map', function () {
			...
			if (document.querySelectorAll) {
				ids = _.map(document.querySelectorAll('#map-test *'), function (n) {
					return n.id;
				});
			...
			*/
			...
			/*
			ids = _.map($('#map-test').children(), function (n) {
				return n.id;
			});
			...
			*/
			...
			/*
			ids = _.map(document.images, function (n) {
				return n.id;
			});
		...
		it('reduceRight', function () {
			...
			list = _.foldr(['foo', 'bar', 'baz'], function (memo, str) {
				return memo + str;
			}, '');
		...
		it('invoke w/ function reference', function () {
			var list = [[5, 1, 7], [3, 2, 1]];
		...
		it('invoke when strings have a call method', function () {
		...
		it('sample', function () {
			...
			equal(allSampled.join(','), numbers.join(','), 'contains the same members before and after sample');
			allSampled = _.sample(numbers, 20).sort();
			equal(allSampled.join(','), numbers.join(','), 'also works when sampling more objects than are present');
