...
app.get('/suggestions', function(req, res) {
    var q = removeDiacritics(req.query.q),
    ...
    function sanitizeResponse(suggestions, q, lat, lng) {
        ...
        _.forEach(suggestions, function(city) {
            ...
            if (city.name.match(whole)) { // city name matches exactly query string
                ...
                city.score = (1+(1-ldist(q,city.name)/city.name.length))/2;
            } else { // city name contains query string or a subset of query string
                city.score = 1-ldist(q,city.name)/city.name.length;
            }

            if (city.population > 2000000) { // big city bias
                city.score += (1-city.score)/(city.population/2000000);
            }
            ...
            if (lat && lng) {
                ...
                if (haversine(start,destination) < 250) { // coordinate biais, within ~300km (computed by approximation from London, ON to Owosso, MI)
...
// compare the score of two suggestions, if equal then sort by population
