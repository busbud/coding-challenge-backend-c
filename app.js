...
// Load the Redis caching functionality as a Middleware for Express
const cacheMiddleware = require('./cache')(config);
...
app.get('/suggestions', cacheMiddleware, (req, res) => {
  ...
  try {
    ...
    if (q.length >= 2) {
      CityModel
          .find({asciiname: {$regex: new RegExp("^" + q, "ig")}}) // Filter based on the string that starts with text in q
          .exec((err, cities) => {
            let results = [];
            if (cities.length > 0) {
