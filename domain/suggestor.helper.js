...
function scoreCity(city, search_term, search_coordinate) {
  ...
  let scoreByName = searchString(city.ascii, search_term).confidence;
  //  get the distance score
  let scoreByDistance = (search_coordinate ? normalizedDistanceBetweenCoordinates(city.coordinate, search_coordinate) : 1);
...
function distanceBetweenCoordinates(coordinate_a, coordinate_b){
  ...
  const phi_1 = coordinate_a.latitude.toRadians();
  const phi_2 = coordinate_b.latitude.toRadians();
  const delta_phi = (coordinate_b.latitude-coordinate_a.latitude).toRadians();
  const delta_lambda = (coordinate_b.longitude-coordinate_a.longitude).toRadians();
...
function normalizedDistanceBetweenCoordinates(coordinate_a, coordinate_b){
  ...
  return (((EARTH_CIRCUMFERENCE/2)-distanceInKm)/(EARTH_CIRCUMFERENCE/2));
}
