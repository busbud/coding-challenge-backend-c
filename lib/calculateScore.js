...
const calculateScoreForAllAndSort = (cities, query, latitude, longitude) => {
  ...
  let qtty = 0;
  cities.forEach((city) => {
    // TODO: this condition is kind of ad-hoc for this case (location existent/inexistent).
    // ideally instead of having two different polynoms we'd have a single
    // set of coefficients and all elements can be present or not.
    // Equation should automatically be adjusted accordingly.
    if(!isNaN(city.weights.location)) {
      ...
      city.score =
        city.weights.population  * 0.6 +
        city.weights.levenshtein * 0.4;
    }
    results.push({
        id: city.value.id,
