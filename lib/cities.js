...
const scoreByDistance = function(city, currentPosition) {
    ...
    return { 
        ...
        score: (1 - (distance / (EARTH_CIRCUM /2))).toFixed(4)
    };
};
        ...
        redisClient.hmget(AUTOCOMP_CITIES_DATA, res, function(err, cityString) {
            ...
            if(cityString != undefined) {
                ...
                cities.forEach(function(city, i) {
                    ...
                    var finalScore = parseFloat(nameScore);
                    
                    var distanceScore = nameScore;
                    if(options.lat !== null && options.long !== null) {
                        var d = scoreByDistance(city, {latitude: options.lat, longitude: options.long});
                        city.distanceFrom = d.distanceFrom.toFixed(2);
                        distanceScore = parseFloat(d.score);
                    }
                    ...
                    console.log(city.name + " : " + nameScore + " " + distanceScore + " " + scorePop + " Final: " + finalScore.toFixed(4));
