...
const getSuggestionsAndPonderate = async (q, latitude, longitude) => {
  ...
  if (result.length) {
    // remove possible duplicates as trie has elements indexed on different keys
    var arrFiltered = result.filter(obj => !uniq[obj.value.id] && (uniq[obj.value.id] = true));
...
const getSanitizedParamsFromQuery(query) {
  const params = {};
  ...
  if ('longitude' in query) {
    params.longitude = parseFloat(query.longitude).toFixed(3); // This should be configurable.
  }
