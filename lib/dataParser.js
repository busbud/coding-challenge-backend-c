...
// load data
async function loadData(fileName = 'cities_canada-usa.tsv', trie, config) {
  const filePath = path.resolve(__dirname, '../data', fileName);
  const stats = await stat(filePath);
  const minPopulation = config.min_city_population ? config.min_city_population : DEFAULT_MIN_POPULATION;
    ...
    return new Promise((resolve, reject) => {
      lineReader.on('line', (line) => {
        ...
        const city = {
          ...
          population: currentRow[COL.POPULATION] !== '' ? parseInt(currentRow[COL.POPULATION], 10) : null,
          score: 0,
        };
