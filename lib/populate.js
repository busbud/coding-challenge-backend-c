...
console.log(rUrl.hostname + ' .. '+rUrl.port);
...
const addToRedis = function(city) {
    ...
    do {
        ...
        redisClient.zadd(AUTOCOMP_CITIES_IDX + ":" + cityComposed.join("").toLowerCase(), 0, city.id);
        index++;
    } while(index <= nLetters);

    redisClient.hset(AUTOCOMP_CITIES_DATA, city.id, JSON.stringify(city));
};
