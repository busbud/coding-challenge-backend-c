...
const REDIS_URL  = process.env.REDISURL || "127.0.0.1";
...
const loadCities = () => {
    ...
    let cityInterface = readline.createInterface({
        input: fs.createReadStream(__dirname + "/../data/cities_canada-usa.tsv")
      });
      
    let header = true;
    
    cityInterface.on('line', (line) => {
        if(header === true) {
            setColumns(line);
            header = false;
        } else {
 ...
 */
const scoreByTyppedLetters = (min, length) =>  {
