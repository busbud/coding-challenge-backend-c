...
export default class SuggestionService {
   ...
   *
   * Fetches all locations starting with or matching the query, or having an identical double metaphone as
   * the query. Then it calculates the similarity between the query and each location's name
   ...
   */
  listSuggestions(query, location) {
    const fLocations = this.locationRepo.listLocations(query, location, this.defaultLimit);
    return fLocations.then((locations) => {
      ...
      return locations
        .map((suggestion, idx) => {
          ...
          /* Penalize results that don't contain the exact text of the query. */
          if (suggestion.fullName.indexOf(query) == -1) {
            //score = score * 0.5;
          }
