...
export const getCitiesSuggestions = (searchParams, citiesSearchableObject, citiesSuggestionsCache) => {
...
const getCachedResponseIfExists = (cacheKey, citiesSuggestionsCache) => {
    if (citiesSuggestionsCache[cacheKey]) {
        return citiesSuggestionsCache[cacheKey];
    }
    return [];
};
const filterCitiesByStringSimilarity = (citiesSearchableObject, searchName) => {
    let filteredCities = [];
...
const filterCitiesByProximity = (filteredCities, searchLatitude, searchLongitude) => {
    const length = filteredCities.length;
    let citiesDistance = {};
    let maxDistance = 0;
    for (var num = 0; num < length; num++) {
        citiesDistance[num] = getDistance({ latitude: searchLatitude, longitude: searchLongitude }, { latitude: filteredCities[num].latitude, longitude: filteredCities[num].longitude });
...
const addToCache = (cacheKey, citiesSuggestions, citiesSuggestionsCache) => {
